	...
	var methods = {
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					...
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
						methods.showMessage.apply(this,["imgErrMsg","Tipo de arquivo incorreto"]);
						continue;
					}
			...
			var getURL = $("<button/>",{
				...
				var image = $('<img/>',{
					src:url,
				  	...
				  	methods.showMessage.apply(this,["imgErrMsg","URL inválida"]); 
				  	return false;
				...
				class:"row-fluid"
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							}).append($('<label/>',{ for:"tblRows"+idExtn,	text:"Linhas"}
							)).append($('<input/>',{
								id:"tblRows"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:2
							})).append($('<label/>',{ for:"tblColumns"+idExtn,	text:"Colunas"}
							)).append($('<input/>',{
								id:"tblColumns"+idExtn,
								type:"text",
							 	class:"form-control form-control-width",
							 	value:2
							})).append($('<label/>',{ for:"tblWidth"+idExtn, text:"Largura"}
							)).append($('<input/>',{
								id:"tblWidth"+idExtn,
								type:"text",
								class:"form-control form-control-width",
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
							}).append($('<label/>',{ for:"tblAlign"+idExtn, text:"Alinhamento"}
							)).append($('<select/>',{ id:"tblAlign"+idExtn, class:"form-control form-control-width"}
								).append($('<option/>',{ text:"Escolha", value:""}
								)).append($('<option/>',{ text:"Esquerda", value:"left"}
								)).append($('<option/>',{ text:"Centro", value:"center"}
								)).append($('<option/>',{ text:"Direita",	value:"right"}))
							).append($('<label/>',{	for:"tblBorder"+idExtn, text:"Tamanho da borda"}
							)).append($('<input/>',{ 
								id:"tblBorder"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"Espaçamento de células"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text", 
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"Preenchimento de células"}
							)).append($('<input/>',{
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
							  ...
							  'styles'	: { "select":true,
											"default": "Formatação",
											"tooltip": "Formatação do Parágrafo",
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											...
											"custom":function(button){
													...
													var palette       = $('<ul />',{id:"color_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Cor do Texto</div>'));
													var bgPalletteDiv = $('<div/>',{id:"bg_colorpellete"});
													var bgPallette    = $('<ul />',{id:"bgcolor_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).html('<div>Cor do Fundo</div>'));
						     ...
						     'ol'		: { "text": "N", 
											"icon": "fa fa-list-ol", 
											"tooltip": "Inserir/Remover lista numerada", 
											"commandname":"insertorderedlist", 
											"custom":null },

						     'ul'		: { "text": "Bullet", 
											"icon": "fa fa-list-ul", 
											"tooltip": "Inserir/Remover lista de bolas", 
						   ...
						   'insert_link': { "modal": true,
											...
											"modalBody": $('<div/>',{   class:"form-group"
																		...
																		id:"inputText",
																		class:"form-control form-control-link ",
																		placeholder:"Texto à ser mostrado",
																	})).append($('<input/>',{
																		type:"text",
																		id:"inputUrl",
																		required:true,
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","Por favor, selecione uma imagem"]);
													return false;
												}
						...
						'insert_table'	: { "modal": true,
						   ...
						   'block_quote': { "text": "Bloco de Comentário", 
											"icon":"fa fa-quote-right", 
											"tooltip": "Bloco de Comentário", 
											"commandname":null, 
											"custom":function(){ 
												methods.setStyleWithCSS.apply(this);
												if(navigator.userAgent.match(/MSIE/i)){													 
						   ...
						   'indent'		: { "text": "Indent", 
											"icon":"fa fa-indent", 
											"tooltip": "Aumentar identação", 
											"commandname":"indent", 
											"custom":null },

						   'outdent'	: { "text": "Outdent", 
											"icon":"fa fa-outdent", 
											"tooltip": "Diminuir identação", 
											"commandname":"outdent", 
											"custom":null },
							...
							'print'		: { "text": "Imprimir", 
											...
											oPrntWin.document.write("<!doctype html><html><head><title>Imprimir</title></head><body onload=\"print();\">" + oDoc.html() + "</body></html>");
											oPrntWin.document.close();
											}},

							'rm_format'	: { "text": "Remover formatação", 
											"icon":"fa fa-eraser", 
											"tooltip": "Remover formatação", 
											"commandname":"removeformat", 
											"custom":null },

							'select_all': { "text": "Selecionar tudo", 
							...
							'splchars'	: { "text": "S", 
											"icon": "fa fa-asterisk", 
											"tooltip": "Inserir caracter especial", 
											"commandname":null, 
											"custom":function(button){
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").html('<div class="label">'+'Palavras : '+wordCount+'</div>');
								            	...
								            	class:"modal-dialog"
							            			...
							            			class:"modal-content"
									            		...
									            		class:"modal-footer"
								                			...
								                			type:"button",
								                			class:"btn btn-success",
								               				}).html('Feito').mousedown(function(e){
								                			e.preventDefault();
